...
draft: false
aliases:
  - "/posts/2025-07/cpp-design-patterns/"
---
...
I was recently browsing [this](https://github.com/amdazm05/libtransform.cc) repository on Github, and was inspired to do something similar. I hadn't written C++ in over 2 years, since taking a course in GPU-Accelerated Computing. But, with my Masters starting in a few months, I decided to start looking in to Modern C++. I noticed this library had some interesting usage of `static`, and it was the first thing I wanted to investigate. I went down a rabbit hole of the static-initialization order, [this](https://www.youtube.com/watch?v=IMZMLvIwa-k&list=LL&index=19) video by The Cherno (I got through my course on OOP 5 years ago because of him!), and Scott Meyers' *Effective C++* (Item 4), where the Meyers Singleton originates, alongside the Factory Creational Pattern. 
...
- Static -> A keyword in C++ that controls the lifetime or visibility of a variable or function:
    ...
    - `static` global variable/function -> This is an unintuitive overload of the keyword that is used outside of a function, in global scope. It keeps the *visibility* of a variable or a function private in its translation unit (the .cpp file). By default, a global variable has **external linkage** meaning it can be shared across the program, in other translation units. By declaring it as static, it is *statically* confined to this one file.
...
P.S. I didn't use the fused multiply-add intrinsic here. GCC rejected it with a `target specific option mismatch` error under what I believed were the correct compile-time flags and includes, and a newer GCC made no difference. I never tracked down the cause, so the kernel below uses a separate multiply and add. 
...
Some notes on these results. As you saw in the custom implementation, I parallelized with OpenMP in such a way that 1 thread would be assigned to 1 element in the result matrix. Without SIMD, this performed quite terribly. Adding SIMD gives a ~13x speedup here, more than the 8x I would expect from moving to 8-wide dot products.

It's also important to note that these are just multiplication times. Both GPU implementations have some *relatively* significant time spent initializing the CUDA context and allocating memory on the GPU. Copying data to and from is relatively fast, around 5-15ms back since it's just a single 1000x1000 matrix. But the initialization (maybe the memory allocation included?) costs around `1000ms`.  
...
I found a comment on The Cherno's video from a senior game engine developer that explains this feeling I had while trying to center system design decisions around the Meyers Singleton. This design, with its reliance on static initialization order, feels fragile. What happens if this library is integrated into a much larger system that has its own `static` nightmares?
